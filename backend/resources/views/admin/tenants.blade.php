                            ...
                            @foreach ($tenants as $tenant)
                                <tr class="hover:bg-gray-50/50">
                                    <td class="px-4 py-3 text-sm font-medium text-gray-900">
                                        {{ $tenant['name'] }}
                                        <div class="text-xs text-gray-400">{{ $tenant['location'] }}</div>
                                    </td>
                                    <td class="px-4 py-3">
                                        <span class="text-xs font-medium px-2 py-0.5 rounded-full {{ $tenant['plan'] === 'Basic' ? 'bg-gray-100 text-gray-600' : ($tenant['plan'] === 'Pro' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700') }}">{{ $tenant['plan'] }}</span>
                                    </td>
                                    <td class="px-4 py-3">
                                        <span class="text-xs font-medium px-2 py-0.5 rounded-full {{ $tenant['status'] === 'Active' ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700' }}">{{ $tenant['status'] }}</span>
                                    </td>
                                    <td class="px-4 py-3 text-sm text-gray-700">{{ $tenant['users'] }}</td>
                                    <td class="px-4 py-3 text-sm text-gray-700">{{ $tenant['products'] }}</td>
                                    <td class="px-4 py-3 text-sm text-gray-700">{{ $tenant['created'] }}</td>
