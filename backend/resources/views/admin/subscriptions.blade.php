        ...
        <div class="p-4 sm:p-6 lg:p-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                <div class="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-1">LKR {{ number_format($subscriptionStats['monthlyRevenue']) }}</p>
                    <p class="text-xs text-emerald-600 mt-2">Up 22.1% vs last month</p>
                </div>
                <div class="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
                    <p class="text-sm text-gray-500">Active Plans</p>
                    <p class="text-2xl font-bold text-gray-900 mt-1">{{ $subscriptionStats['activePlans'] }}</p>
                    <p class="text-xs text-blue-600 mt-2">Up 15.2% vs last month</p>
                </div>
                <div class="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
                    <p class="text-sm text-gray-500">Expiring Soon</p>
                    <p class="text-2xl font-bold text-gray-900 mt-1">{{ $subscriptionStats['expiringSoon'] }}</p>
                </div>
                <div class="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
                    <p class="text-sm text-gray-500">Avg. Revenue/Tenant</p>
                    <p class="text-2xl font-bold text-gray-900 mt-1">LKR {{ number_format($subscriptionStats['avgRevenuePerTenant']) }}</p>
                    ...
                    @foreach ($revenueByPlan as $plan)
                        <div>
                            <div class="flex items-center justify-between text-sm">
                                ...
                                <span class="font-semibold">LKR {{ number_format($plan['revenue']) }}</span>
                            </div>
                            <div class="mt-2 w-full bg-gray-100 rounded-full h-2">
                                <div class="bg-blue-600 h-2 rounded-full" style="width: {{ min(100, round($plan['revenue'] / 4000)) }}%"></div>
                            </div>
                        </div>
                    @endforeach
                </div>
                        ...
                        <tbody class="divide-y divide-gray-50">
                            @foreach ($subscriptions as $sub)
                                <tr class="hover:bg-gray-50/50">
                                    <td class="px-4 py-3 text-sm font-medium text-gray-900">{{ $sub['company'] }}</td>
                                    <td class="px-4 py-3">
                                        <span class="text-xs font-medium px-2 py-0.5 rounded-full {{ $sub['plan'] === 'Basic' ? 'bg-gray-100 text-gray-600' : ($sub['plan'] === 'Pro' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700') }}">{{ $sub['plan'] }}</span>
                                    </td>
                                    <td class="px-4 py-3 text-sm font-semibold">LKR {{ number_format($sub['amount']) }}</td>
                                    <td class="px-4 py-3">
                                        <span class="text-xs font-medium px-2 py-0.5 rounded-full {{ $sub['status'] === 'Active' ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700' }}">{{ $sub['status'] }}</span>
                                    </td>
                                    <td class="px-4 py-3 text-sm text-gray-700">{{ $sub['nextBilling'] }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
